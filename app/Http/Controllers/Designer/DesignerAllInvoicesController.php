...
class DesignerAllInvoicesController extends DesignerBaseController
{
    ...
    public function index() {
        ...
        return view('designer.invoices.index', $this->data);
    }
    ...
    public function data(Request $request) {
        ...
        $invoices = $invoices->whereNull('projects.deleted_at')->orderBy('invoices.id', 'desc')->get();
        ...
        return DataTables::of($invoices)
            ->addColumn('action', function ($row) use ($firstInvoice) {
                ...
                if($this->user->can('view_invoices')){
                    $action.= '<li><a href="' . route("designer.all-invoices.download", $row->id) . '"><i class="fa fa-download"></i> Download</a></li>';
                }
                if($row->status == 'paid')
                {
                ...
                {
                    $action .= '<li><a href="' . route("designer.all-invoices.edit", $row->id) . '"><i class="fa fa-pencil"></i> Edit</a></li>';
                }

                if($row->status == 'unpaid'){
                    $action.= '<li><a href="'.route("designer.payments.payInvoice", [$row->id]).'" data-toggle="tooltip" ><i class="fa fa-plus"></i> '.__('modules.payments.addPayment').'</a></li>';
                }
            ...
            ->editColumn('invoice_number', function ($row) {
                return '<a href="' . route('designer.all-invoices.show', $row->id) . '">' . ucfirst($row->invoice_number) . '</a>';
            })
...
//       return $pdf->stream();
    ...
    public function create() {
        ...
        return view('designer.invoices.create', $this->data);
    }
    ...
    public function store(StoreInvoice $request)
    {
        ...
        foreach ($items as $key => $item):
            if(!is_null($item)){
                InvoiceItems::create(['invoice_id' => $invoice->id, 'item_name' => $item, 'item_summary' => $itemsSummary[$key], 'type' => 'item', 'quantity' => $quantity[$key], 'unit_price' => round($cost_per_item[$key], 2), 'amount' => round($amount[$key], 2), 'tax_id' => $taxes[$key]]);
            }
        endforeach;
        ...
        return Reply::redirect(route('designer.all-invoices.index'), __('messages.invoiceCreated'));
    ...
    public function edit($id) {
        ...
        return view('designer.invoices.edit', $this->data);
    }
    ...
    public function update(StoreInvoice $request, $id)
    {
        ...
        return Reply::redirect(route('designer.all-invoices.index'), __('messages.invoiceUpdated'));
    ...
    public function show($id){
        ...
        return view('designer.invoices.show', $this->data);
    }
    ...
    public function convertEstimate($id) {
        ...
        return view('designer.invoices.convert_estimate', $this->data);
    }
    ...
    public function addItems(Request $request)
    {
        ...
        $this->taxes = Tax::all();
        $view = view('designer.invoices.add-item', $this->data)->render();
    ...
    public function paymentDetail($invoiceID)
    {
        ...
        return View::make('designer.invoices.payment-detail', $this->data);
    }
