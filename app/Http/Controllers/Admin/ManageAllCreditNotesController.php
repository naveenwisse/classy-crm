...
//        dd('test');
        ...
        return DataTables::of($creditNotes)
            ...
            ->addColumn('action', function ($row) use($firstCreditNotes){
                ...
                <ul role="menu" class="dropdown-menu">
                    <li><a href="' . route("admin.all-credit-notes.download", $row->id) . '"><i class="fa fa-download"></i> Download</a></li>';

                $action .= ' <li><a href="javascript:" data-credit-notes-id="' . $row->id . '" class="credit-notes-upload" data-toggle="modal" data-target="#creditNoteUploadModal"><i class="fa fa-upload"></i> Upload </a></li>';

                $action .= '<li><a href="' . route("admin.all-credit-notes.edit", $row->id) . '"><i class="fa fa-pencil"></i> Edit</a></li>';
    ...
    public function store(StoreCreditNotes $request)
    {
        ...
        //log search
        $this->logSearchEntry($creditNote->id, 'CreditNote ' . $creditNote->cn_number, 'admin.all-credit-notes.show');
    ...
    public function export($startDate, $endDate, $projectID)
    {
        ...
        ->map(function($cn) {
            return [
                ...
                'issue_date' => $cn->issue_on
            ];
        })->toArray();

        // Define the Excel spreadsheet headers
        $headerRow = ['ID', 'CreditNote #', 'Project Name', 'Status', 'Total Amount', 'Invoice Date'];
