...
class SendEventReminder extends Command
{
    ...
    public function calculateReminderDateTime(Event $event)
    {
        ...
        switch ($type) {
            case 'day':
                $reminderDateTime = Carbon::parse($event->start_date_time, global_setting()->timezone)->subDays($time);
                break;
            case 'hour':
                $reminderDateTime = Carbon::parse($event->start_date_time, global_setting()->timezone)->subHours($time);
                break;
            case 'minute':
                $reminderDateTime = Carbon::parse($event->start_date_time, global_setting()->timezone)->subMinutes($time);
                break;
        }
