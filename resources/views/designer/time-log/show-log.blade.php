    ...
    <div class="row bg-title">
        ...
        <div class="col-lg-6 col-sm-8 col-md-8 col-xs-12">
            <ol class="breadcrumb">
                ...
                <li><a href="{{ route('designer.projects.index') }}">{{ $pageTitle }}</a></li>
            ...
            <section>
                <div class="sttabs tabs-style-line">
                    <div class="white-box">
                        <nav>
                            <ul>
                                <li><a href="{{ route('designer.projects.show', $project->id) }}"><span>@lang('modules.projects.overview')</span></a>
                                </li>

                                @if(in_array('employees',$modules))
                                <li><a href="{{ route('designer.project-members.show', $project->id) }}"><span>@lang('modules.projects.members')</span></a></li>
                                @endif

                                @if(in_array('tasks',$modules))
                                <li><a href="{{ route('designer.tasks.show', $project->id) }}"><span>@lang('app.menu.tasks')</span></a></li>
                                @endif

                                <li><a href="{{ route('designer.files.show', $project->id) }}"><span>@lang('modules.projects.files')</span></a></li>
                                @if(in_array('timelogs',$modules))
                                <li class="tab-current"><a href="{{ route('designer.time-log.show-log', $project->id) }}"><span>@lang('app.menu.timeLogs')</span></a></li>
                                @endif
                            </ul>
                        </nav>
                    </div>
...
<script>
    var table = $('#timelog-table').dataTable({
        ...
        processing: true,
        serverSide: true,
        ajax: '{!! route('designer.time-log.data', $project->id) !!}',
        deferRender: true,
        language: {
...
//        console.log(hours+" "+minutes);
    ...
    $('#save-form').click(function () {
        $.easyAjax({
            url: '{{route('designer.time-log.store-time-log')}}',
            container: '#logTime',
            type: "POST",
    ...
    $('body').on('click', '.sa-params', function(){
        ...
        swal({
            ...
            if (isConfirm) {
                ...
                var url = "{{ route('designer.all-time-logs.destroy',':id') }}";
...
//                                    swal("Deleted!", response.message, "success");
    ...
    $('body').on('click', '.edit-time-log', function () {
        ...
        var url = '{{ route('designer.time-log.edit', ':id')}}';
        url = url.replace(':id', id);
