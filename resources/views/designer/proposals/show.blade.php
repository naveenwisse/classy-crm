            ...
            <section>
                <div class="sttabs tabs-style-line">
                    <div class="white-box">
                        <nav>
                            <ul>
                                <li ><a href="{{ route('designer.leads.show', $lead->id) }}"><span>@lang('modules.lead.profile')</span></a>
                                </li>
                                @if($user->can('edit_lead'))
                                    <li class="tab-current"><a href="{{ route('designer.proposals.show', $lead->id) }}"><span>@lang('modules.lead.proposal')</span></a></li>
                                    <li ><a href="{{ route('designer.lead-files.show', $lead->id) }}"><span>@lang('modules.lead.file')</span></a></li>
                                    <li ><a href="{{ route('designer.leads.followup', $lead->id) }}"><span>@lang('modules.lead.followUp')</span></a></li>
                                @endif
                            </ul>
                        </nav>
                    </div>
                                                    ...
                                                    <div class="row">
                                                        <div class="col-sm-6">
                                                            <div class="form-group">
                                                                <a href="{{ route('designer.proposals.create') }}/{{$lead->id}}" class="btn btn-outline btn-success btn-sm">@lang('modules.proposal.createTitle') <i class="fa fa-plus" aria-hidden="true"></i></a>
                                                            </div>
                                                        </div>
                                                    </div>
...
<script>
    ...
    var table = $('#proposal-table').dataTable({
        ...
        processing: true,
        serverSide: true,
        ajax: '{!! route('designer.proposals.data', $lead->id) !!}',
        "order": [[ 0, "desc" ]],
        language: {
    ...
    $('body').on('click', '.sa-params', function(){
        ...
        swal({
            ...
            if (isConfirm) {
                ...
                var url = "{{ route('designer.proposals.destroy',':id') }}";
