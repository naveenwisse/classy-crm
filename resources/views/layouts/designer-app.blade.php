    ...
    <nav class="navbar navbar-default navbar-static-top m-b-0">
        <div class="navbar-header">
            ...
            <div class="top-left-part">
                <!-- Logo -->
                <a class="logo hidden-xs hidden-sm text-center" href="{{ route('designer.dashboard') }}">
                    <!-- Logo icon image, you can use font-icon also -->

                        <!--This is dark logo icon-->
                            <img src="{{ $global->logo_url }}" alt="home" class=" member-logo" />
...
<script>
    ...
    $('body').on('click', '.active-timer-modal', function(){
        var url = '{{ route('designer.all-time-logs.show-active-timer')}}';
    ...
    function addOrEditStickyNote(id)
    {
        ...
        if(id === undefined || id == "" || id == null) {
            url =  '{{ route('designer.sticky-note.store') }}'
        } else{

            url = "{{ route('designer.sticky-note.update',':id') }}";
    ...
    function showCreateNoteModal(){
        var url = '{{ route('designer.sticky-note.create') }}';
    ...
    function showEditNoteModal(id){
        var url = '{{ route('designer.sticky-note.edit',':id') }}';
        url  = url.replace(':id',id);
    ...
    function deleteSticky(id){
        ...
        swal({
            ...
            if (isConfirm) {
                ...
                var url = "{{ route('designer.sticky-note.destroy',':id') }}";
                url = url.replace(':id', id);
    ...
    function getNoteData(){
        ...
        var url = "{{ route('designer.sticky-note.index') }}";
