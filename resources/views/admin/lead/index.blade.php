      ...
      function tableLoad() {
          var client = $('#client').val();
           ...
           table = $('#users-table').dataTable({
              ...
              destroy: true,
              stateSave: true,
              ajax: '{!! route('admin.leads.data') !!}?client='+client,
              language: {
...
//                        table._fnDraw();
        ...
        $(document).ready(function(){
            $('body').on('click', '.btn_convert',function () {
                let lead_id = $(this).data('id');
                swal({
                    title: "Are you sure?",
                    text: "Do you want to change lead status to \"Converted Sale\"?",
                    type: "warning",
                    showCancelButton: true,
                    confirmButtonColor: "#DD6B55",
                    ...
                    if (isConfirm) {
                        var url = '{{ route("admin.clients.create") }}' + '/' + lead_id + '/' + 'convert';
                        document.location.href = url;
                    }
                    else{
                        var url = '{{ route("admin.clients.create") }}' + '/' + lead_id;
                        document.location.href = url;
                    }
                });
            });

            $('body').on('click', '.btn_convert_project',function () {
                let lead_id = $(this).data('id');
                swal({
                    title: "Warning!",
                    text: "This lead needs to be converted to a client before you can add a project.",
                    type: "warning",
                    confirmButtonColor: "#DD6B55",
