...
<div id="container">
    ...
    <section id="creditNote-info">
        <div>
            <span>@lang('app.menu.issues') @lang('app.date'):</span> <span>{{ $creditNote->issue_date->format("dS M Y") }}</span>
        </div>
        @if($creditNote->status == 'unpaid')
        <div>
            <span>@lang('app.dueDate'):</span> <span>{{ $creditNote->due_date->format("dS M Y") }}</span>
        </div>
        @endif
        @if($invoiceNumber)
            <div>
