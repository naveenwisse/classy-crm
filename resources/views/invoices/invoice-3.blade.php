...
<div id="container">
    ...
    @if(!is_null($invoice->project->client))
        <section id="client-info">
            ...
            <div>
                <span>{{ ucwords($invoice->project->clientdetails->company_name) }}</span>
            </div>
            ...
            <div>
                ...
                @if($invoiceSetting->show_gst == 'yes' && !is_null($invoice->project->clientdetails->gst_number))
                    <div>
                        <span> @lang('app.gstIn'): {{ $invoice->project->clientdetails->gst_number }} </span>
                    </div>
                @endif
            @endif
    ...
    <section id="sums">
        ...
        <table cellpadding="0" cellspacing="0">
            ...
            <tr>
                ...
                <td>{!! htmlentities($invoice->currency->currency_symbol)  !!} @if($invoice->status == 'paid' && $paidAmount == 0){{ $invoice->total }} @else {{ $paidAmount }} @endif</td>
            </tr>
            <tr>
                <th>@lang("modules.invoices.total") @lang("modules.invoices.due"):</th>
                <td>{!! htmlentities($invoice->currency->currency_symbol)  !!} @if($invoice->status == 'paid' && $paidAmount == 0) 0 @else {{ max(($invoice->total-$paidAmount),0) }} @endif</td>
            </tr>
    ...
    <section id="invoice-info">
        <div>
            <span>@lang('app.menu.issues') @lang('app.date'):</span> <span>{{ $invoice->issue_date->format("dS M Y") }}</span>
        </div>
        @if($invoice->status == 'unpaid')
        <div>
            <span>@lang('app.dueDate'):</span> <span>{{ $invoice->due_date->format("dS M Y") }}</span>
        </div>
        @endif
        <div>
